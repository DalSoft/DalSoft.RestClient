...
namespace DalSoft.RestClient.Test.Unit.Handlers
{
    ...
    public class RetryHandlerTests
    {
        ...
        [TestCase(1)]
        [TestCase(601)]
        public void Ctor_PassWaitToRetryInSecondsLessThan2OrGreaterThen600_ThrowsArgumentException(int waitToRetryInSeconds)
        {
        ...
        {
            var retryCount = 1;
            const int waitToRetryInSeconds = 2;

            var retryHandler = new RetryHandler(3, waitToRetryInSeconds, 60, RetryHandler.BackOffStrategy.Exponential)
            {
                BackOffFunc = seconds =>
                {
